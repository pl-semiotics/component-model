...
let atmap (f : 'a -> 'b) : 'a Source.phrase -> 'b Source.phrase
  = atmap' (fun x r -> f x)
(*  = { Source.it = f x.Source.it; Source.at = x.Source.at; }*)
...
(*let anon category space n =
  let open Putil in let open Source in
...
let lookup_here ctx sort x =
  try Some (VarMap.find x.it (space_of_sort ctx sort).map)
  with Not_found -> None
let rec lookup_opt
...
type 'a ctxops =
  ...
  ; _bind : 'a ctx -> sort -> V.binder -> ()
  ; _bind_type : 'a ctx -> V.binder -> I.def_type -> ()
  ; _bind_core_type : 'a ctx -> V.binder -> I.core_deftype_ -> ()
  }
let def_ctxops =
  { _desugar_ident = desugar_ident
  ; _bind = bind
  ; _bind_type = bind_type
  ; _bind_core_type = bind_core_type
  }
 *)
...
and desugar_val_type (ctx : 'a ctx) (params : 'a desugar_type_params) at
  ...
  | V.TInline t -> let t' = desugar_def_val_type ctx params t in
                   params._deinline_type ctx (I.Deftype_val t' @@ no_region)
...
(*let rec desugar_instance_decl' ctx params (decl : V.instance_decl')
        : I.instance_decl'
  =  match decl with*)
...
let rec desugar_type_bound' ctx params (tc : V.type_bound') at : I.type_bound'
  = match tc with
  | V.Tbound_subr -> I.Tbound_subr
  | V.Tbound_eq dt ->
     match dt with
     | V.TVar id -> I.Tbound_eq (params._desugar_ident ctx (Type @@ at) id)
     | V.TInline t -> let t' = desugar_def_type ctx params None t in
                      I.Tbound_eq (params._deinline_type ctx t')
and desugar_type_bound ctx params (tb : V.type_bound) : I.type_bound
  = atmap' (desugar_type_bound' ctx params) tb
(*and desugar_type_bound
    : 'c. 'c ctx -> 'c desugar_type_params -> V.type_bound -> I.type_bound
  = fun ctx params -> atmap (desugar_type_bound' ctx params)*)
...
let rec desugar_definition' (ctx : definition_ctx) (def : V.definition') at
  ...
  | V.CanonDef bc ->
     ...
     let sort = match c.it with
       ...
       | I.Canon_lower _ -> CoreSort (Core_func @@ c.at) @@ c.at in
...
(*d's @ (d' :: desugar_definitions ctx defs)*)
