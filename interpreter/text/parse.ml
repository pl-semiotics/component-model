...
let parse' name lexbuf start =
  ...
  with Syntax (region, s) ->
    let region' = if region <> Wasm.Source.no_region then region else
      {Wasm.Source.left = Lexer.convert_pos lexbuf.Lexing.lex_start_p;
       Wasm.Source.right = Lexer.convert_pos lexbuf.Lexing.lex_curr_p} in
  ...
  = let lcm_status : int ref = ref 0
    ...
    let (x, d) =
      Wasm.Parser.module1 (fun lexbuf ->
          print_endline ("lexbuf " ^ string_of_int lexbuf.Lexing.lex_curr_pos);
          if !paren_depth = 0
          then match !lcm_status with
               ...
               | Wasm.Parser.RPAR -> paren_depth := !paren_depth - 1;
                                     Wasm.Parser.RPAR
               | tok -> print_endline ("-- allow tok"); tok) lexbuf
...
let lex_core_deftype (lexbuf : Lexing.lexbuf)
  ...
  = let lcm_status : int ref = ref 0
    ...
    let prim_lex () =
      ...
      | Wasm.Parser.RPAR -> paren_depth := !paren_depth - 1;
                            Wasm.Parser.RPAR
      | tok -> print_endline ("-- allow tok"); tok in
    let ty =
