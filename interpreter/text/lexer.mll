...
rule token = parse
  | "(" { print_endline "-- lex ("; LPAR }
  | ")" { print_endline "-- lex )"; RPAR }
  ...
  | name as s { print_endline "-- lex name"; VAR s }
...
(*and core = parse
  | space#'\n' { core lexbuf }
  | '\n' { Lexing.new_line lexbuf; core lexbuf }
  | "instance" { print_endline "-- coreinstance"; COREINSTANCE }
  | "alias" { print_endline "-- corealias"; COREALIAS }
  | "func" { print_endline "-- corefunc"; COREFUNC }
  | "module" { print_endline "-- coremodule"; COREMOD (lex_core_module lexbuf) }*)
