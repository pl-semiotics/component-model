...
let bsubst_subst (bs : bsubst) : subst
  = { bvar_sub = bs ; evar_sub = [] ; uvar_sub = [] }
let esubst_subst (es : esubst) : subst
  = { bvar_sub = [] ; evar_sub = es ; uvar_sub = [] }
let usubst_subst (us : usubst) : subst
  = { bvar_sub = [] ; evar_sub = [] ; uvar_sub = us }
...
let rec bound_to_uvars (ctx : ctx) (vs : boundedtyvar list) : ctx * bsubst
  = match vs with
  | [] -> (ctx, [])
  | v::vs' ->
     let ctx', sub = bound_to_uvars { ctx with uvars = ctx.uvars @ [v] } vs' in
...
and subst_def_type (s : subst) (dt : def_type) : def_type
  ...
  | DT_var (TV_free (FTV_uvar (o, i))) ->
     (try match List.nth (List.nth s.uvar_sub o) i with
          | None -> dt
          | Some dt' -> dt'
      with Not_found -> dt)
  | DT_var (TV_free (FTV_evar (o, i))) ->
     (try match List.nth (List.nth s.evar_sub o) i with
          | None -> dt
          | Some dt' -> dt'
      with Not_found -> dt)
